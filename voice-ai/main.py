...
# RECENT_MESSAGES: number of recent messages sent verbatim to LLM
RECENT_MESSAGES = 6
# SUMMARIZE_THRESHOLD: when total messages exceed this, older ones get summarized
SUMMARIZE_THRESHOLD = 10
# SUMMARY_MODEL: model used for summarization (cheap/fast)
...
async def _maybe_summarize(conv_id: int, conv: dict) -> str | None:
    """If conversation is long enough, summarize older messages and cache.
    Returns the summary string (possibly empty if not enough messages)."""
    ...
    # Keep the most recent RECENT_MESSAGES out of summarization
